#!/bin/bash
# Test the apply step with simulated "already exists" errors
# This simulates what happens when imports don't persist
...
echo "🧪 Testing apply step with simulated 'already exists' scenario..."
...
echo "📋 Step 1: Creating fresh terraform plan..."
terraform plan \
...
# Step 2: Simulate the scenario where imports didn't persist
# Remove a resource from state (but it still exists in AWS)
echo "🔍 Step 2: Simulating 'already exists' scenario..."
echo "  Removing a resource from state to simulate import failure..."
echo "  (In real scenario, this would happen because import didn't persist)"

# Pick a resource that likely exists in AWS
TEST_RESOURCE="aws_ecr_repository.app"
if terraform state list 2>/dev/null | grep -q "^${TEST_RESOURCE}$"; then
  echo "  → Removing $TEST_RESOURCE from state (but it still exists in AWS)..."
  terraform state rm "$TEST_RESOURCE" 2>/dev/null || true
  echo "  ✅ Removed from state"
else
  echo "  → $TEST_RESOURCE not in state (this is fine for testing)"
fi

# Regenerate plan - this will try to create the resource again
echo ""
echo "  Regenerating plan (will try to create removed resource)..."
terraform plan \
  -var-file="terraform.tfvars" \
  -out=tfplan >/dev/null 2>&1 || true

# Step 3: Test apply with error handling
echo ""
echo "🚀 Step 3: Testing apply with error handling..."
echo "  This should fail with 'already exists' and trigger re-import..."
echo ""
...
terraform apply -auto-approve tfplan 2>&1 | tee /tmp/apply_output.log || {
  ...
  if grep -qE "(already exists|AlreadyExists|ResourceAlreadyExists|EntityAlreadyExists|BucketAlreadyExists|RepositoryAlreadyExistsException|DBParameterGroupAlreadyExists|ResourceInUseException)" /tmp/apply_output.log; then
    echo ""
    echo "✅ Correctly detected 'already exists' error!"
...
echo ""
echo "Note: This test simulated the 'already exists' scenario by removing"
echo "      a resource from state. In GitHub Actions, this would happen"
echo "      when imports don't persist to the remote backend."
