...
@pytest.mark.asyncio
async def test_run_job(job_service, mock_job_repository, mock_job_run_repository, mock_message_queue):
    """Test running a job."""
    job_id = uuid4()
    job = JobDefinition(
        id=job_id,
...
@pytest.mark.asyncio
async def test_run_job_no_queue(job_service, mock_job_repository, mock_job_run_repository, mock_job_runner):
