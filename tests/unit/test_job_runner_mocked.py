...
async def test_execute_export_job_success(
    ...
    mock_query_engine.execute_export_query = AsyncMock(return_value={
        "count": 5,
        "records": [
            {"id": "1", "amount": 100.0},
            {"id": "2", "amount": 200.0},
    ...
    with patch("app.services.job_runner.FileGenerator.generate_csv_file", return_value="/tmp/export.csv") as mock_gen_csv, \
         patch("app.services.job_runner.FileGenerator.get_file_extension", return_value=".csv") as mock_get_ext, \
         patch("app.services.job_runner.FileGenerator.get_content_type", return_value="text/csv") as mock_get_ct, \
         patch("os.makedirs"), \
         patch("os.remove"):
...
async def test_execute_import_job_success(
    ...
    # Mock SaaS client
    mock_saas_client.fetch_data = AsyncMock(return_value=[
        {"id": "1", "amount": 100.0},
        {"id": "2", "amount": 200.0},
    ])
    mock_saas_client.import_data = AsyncMock(return_value={
        "imported_count": 2,
        "failed_count": 0,
    })
...
async def test_execute_job_run_failure(job_runner, mock_job_run_repository):
    ...
    with patch.object(
        job_runner, "_execute_export_job", new_callable=AsyncMock, side_effect=Exception("Export failed")
    ) as mock_execute:
        await job_runner._execute_job_run(job, job_run, "worker-0")
        
        # Verify status was updated to FAILED
        update_calls = [call for call in mock_job_run_repository.update_status.call_args_list]
